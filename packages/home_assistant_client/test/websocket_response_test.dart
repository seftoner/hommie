...
import 'package:home_assistant_client/src/protocol/responses/web_socket_response.dart';
import 'package:home_assistant_client/src/protocol/types/hass_event.dart';
import 'package:home_assistant_client/src/protocol/types/hass_service.dart';
...
void main() {
  test('Deserialization of event reposne ', () async {
    final testJson = await readJsonTestDataFromFile(
      'test/data_samples/event_initial.json',
    );
    ...
    expect(
      state.add?.containsKey('binary_sensor.ivano_frankivska_oblast_artillery'),
      isTrue,
    );
  });
  ...
  test('Event with changes', () async {
    ...
    expect(
      state.change!['fan.philips_purifier']?.add?.last_updated,
      1712828878.514165,
    );
    expect(
      state.change!['fan.philips_purifier']?.add?.context,
  ...
  test('Hass servcies deserialization - short', () async {
    ...
    final responseResult = switch (response) {
      WebSocketResultResponseSuccess(:final result) => HassServices.fromJson(
        result,
      ),
  ...
  test('Error response deserialization', () async {
    final testJson = await readJsonTestDataFromFile(
      'test/data_samples/error_response.json',
    );
  ...
  test('callService response parsing', () async {
    ...
    expect(
      responseResult.context.id,
      equals('326ef27d19415c60c492fe330945f954'),
    );
    expect(responseResult.context.parent_id, isNull);
    expect(
      responseResult.context.user_id,
